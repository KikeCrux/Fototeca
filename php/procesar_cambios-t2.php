...
$sql = "SELECT * FROM Asignado WHERE ID_Asignado = $id_asignado";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE Asignado SET Nombre='$nuevo_nombre', PuestoDepartamento='$nuevo_puesto', Observaciones='$nuevas_observaciones' WHERE ID_Asignado=$id_asignado";

    if ($conn->query($sql) === TRUE) {
        // Redirigir a la página de cambios con un mensaje de éxito
        header("Location: cambio-t2.php?success_message=Cambios realizados exitosamente.");
        exit();
    } else {
...
<body>
    ...
    <div class="container mt-5">
        <h1 class="text-center">Procesar Cambios de Asignado</h1>
        <br>
        <?php if (!empty($error_message)) : ?>
            <div class="alert alert-danger text-center"><?php echo $error_message; ?></div>
        <?php endif; ?>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id_asignado; ?>" method="post">
